...
import com.panahit.telegramma.time.FastDateFormat;
...
public class LocaleController {
    ...
    public CalendarInfo currentCalendarInfo;
    ...
    public static class CalendarInfo {
        public int name;
        public String nameEnglish;
        public String icuTag;

        public String getSaveString() {
            return name + "|" + nameEnglish + "|" + icuTag;
        }

        public static CalendarInfo createWithString(String string) {
            if (string == null || string.length() == 0) {
                return null;
            }
            String[] args = string.split("\\|");
            ...
            calendarInfo.name = Integer.parseInt(args[0]);
            calendarInfo.nameEnglish = args[1];
            calendarInfo.icuTag = args[2];
            return calendarInfo ;
        }
    }
    ...
    public ArrayList<CalendarInfo> sortedCalendars = new ArrayList<>();
    public HashMap<String, CalendarInfo> calendarsDict = new HashMap<>();
    ...
    public LocaleController() {
        ...
        CalendarInfo calendarInfo = new CalendarInfo();
        calendarInfo.name = R.string.GregorianCalendar;
        calendarInfo.nameEnglish = "Gregorian";
        calendarInfo.icuTag = "gregorian";
        sortedCalendars.add(calendarInfo);
        calendarsDict.put(calendarInfo.icuTag, calendarInfo);

        calendarInfo = new CalendarInfo();
        calendarInfo.name = R.string.PersianCalendar;
        calendarInfo.nameEnglish = "Persian";
        calendarInfo.icuTag = "persian";
        sortedCalendars.add(calendarInfo);
        calendarsDict.put(calendarInfo.icuTag, calendarInfo);
        ...
        try {
            ...
            applyCalendar(currentCalendar, override);
        } catch (Exception e) {
            FileLog.e("tmessages", e);
        }
    ...
    public void applyCalendar(CalendarInfo calendarInfo, boolean override) {
        ...
        try {
            ...
            newULocale = new ULocale(lang + "@calendar=" + calendarInfo.icuTag);
            if (newULocale != null) {
                if (override) {
                    SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
                    SharedPreferences.Editor editor = preferences.edit();
                    editor.putString("calendar", calendarInfo.icuTag);
                    editor.commit();
                }
                currentULocale = newULocale;
            }
    ...
    public static String getCurrentCalendarName() {
        return LocaleController.getString("Gregorain", LocaleController.getInstance().currentCalendarInfo.name);
    }
    ...
    public static String formatDateChat(long date) {
        Calendar rightNow = Calendar.getInstance(LocaleController.getInstance().currentULocale);
    ...
    private FastDateFormat createFormatter(ULocale locale, String format, String defaultFormat) {
    ...
    public void recreateFormatters() {
        ...
        ULocale uLocale = currentULocale;
        if (locale == null) {
            locale = Locale.getDefault();
        }
        if (uLocale == null) {
            uLocale = new ULocale("en", "US");
        }
        ...
        formatterMonth = createFormatter(uLocale, getStringInternal("formatterMonth", R.string.formatterMonth), "dd MMM");
        formatterYear = createFormatter(uLocale, getStringInternal("formatterYear", R.string.formatterYear), "dd.MM.yy");
        formatterYearMax = createFormatter(uLocale, getStringInternal("formatterYearMax", R.string.formatterYearMax), "dd.MM.yyyy");
        chatDate = createFormatter(uLocale, getStringInternal("chatDate", R.string.chatDate), "d MMMM");
        chatFullDate = createFormatter(uLocale, getStringInternal("chatFullDate", R.string.chatFullDate), "d MMMM yyyy");
        formatterWeek = createFormatter(uLocale, getStringInternal("formatterWeek", R.string.formatterWeek), "EEE");
        formatterMonthYear = createFormatter(uLocale, getStringInternal("formatterMonthYear", R.string.formatterMonthYear), "MMMM yyyy");
        formatterDay = createFormatter(lang.toLowerCase().equals("ar") || lang.toLowerCase().equals("ko") ? uLocale : ULocale.US, is24HourFormat ? getStringInternal("formatterDay24H", R.string.formatterDay24H) : getStringInternal("formatterDay12H", R.string.formatterDay12H), is24HourFormat ? "HH:mm" : "h:mm a");
    }
